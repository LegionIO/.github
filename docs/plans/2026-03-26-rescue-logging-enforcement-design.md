...
- **False positives**: The Tier 2 script may flag rescue blocks where logging happens in a called method (e.g., `handle_error(e)` which internally logs). The `notice` severity and allowlist mechanism mitigate this.
